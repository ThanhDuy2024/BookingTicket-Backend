...
import { categoriesDetailService, categoriesService, deleteCategoryService, getCategoriesService, putCategoriesService } from "../../services/admins/categories.service";
...
export const putCategoriesController = async (req: admin, res: Response) => {
  try {
    if(req.file) {
      req.body.image = req.file.path;
    } else {
    ...
    const data: CategoriesDto = req.body;
    const message = await putCategoriesService(data, req.admin.id, String(id));
...
export const deleteCategoriesController = async (req: admin, res: Response) => {
  try {
    const id = req.params.id;
    const message = await deleteCategoryService(String(id), req.admin.id);
    return res.status(message.status).json({
      code: message.code,
      message: message.message,
    })
